...
This folder contains two files, a ``.yaml`` file, which defines the ram, and a ``metadata.txt`` file which ``pinefarm`` will use to add extra key-value pairs to the final grids.
